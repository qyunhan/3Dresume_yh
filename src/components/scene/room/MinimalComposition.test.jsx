...
test('keeps exactly the prescribed project, career, and school heroes in one connected room composition', () => {
  ...
  expect(school.props.items.map((item) => item.id)).toEqual(['nus', 'rc4-flag', 'ucla'])
  ...
  expect(shelf?.querySelectorAll('group[name="NUS"], group[name="RC4 Flag"], group[name="UCLA Exchange"]')).toHaveLength(3)
  expect(container.querySelectorAll('group[name="Science Club"], group[name="About this room"]')).toHaveLength(0)
})
